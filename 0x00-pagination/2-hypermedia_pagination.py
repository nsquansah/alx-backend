...
class Server:
    ...
    def get_hyper(self, page: int = 1, page_size: int = 10) -> Dict:
        """Returns paginated items as Dict, with extra details
        """
        paginated_pages = self.get_page(page, page_size)
        total_pages = len(self.dataset())
        prev_page = (page - 1) if (page - 1) > 0 else None
        next_page = (page + 1) if (total_pages >= page * page_size) else None
        hyper_data = {
                'page_size': paginated_pages,
                'page': page,
